...
// pub struct FmtModule<'a>(&'a Module);
impl Module {
    pub fn formatter(&self)->ParsedModule{
        let source_code = self.to_string();
        let parsed = parse(&source_code, ParseOptions::from(Mode::Module)).unwrap();
